...
if ($avatarSyncHistory -EQ $nothing) {
    ...
    Write-Output "Sync has expired. Over-write existing avatar image."
    # We might need to delete the existing image for this to work correctly. Keep an eye on this and 
...
if ($profilePicturesFolder -EQ $Nothing) { 
    ...
    if ($profilePicturesFolder -EQ $Nothing) {
        ...
        # In this case, the folder needs to be 'published'. 
...
try {
    ...
    $form = @{
        ...
        size = $fileSize
        # If CONTENT_TYPE is omitted it will be guessed based on file extension. 
