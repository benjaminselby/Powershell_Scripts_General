...
param (
    ...
    [string]$token      = '<TOKEN>',
    [hashtable]$headers    = @{Authorization="Bearer $token"},

    [string]$rootFolder     = "$(Split-Path $MyInvocation.MyCommand.path)",
    [string]$workingFolder  = "$rootFolder\Data",
    [string]$initFilePath   = "$rootFolder\enrol_init.json",
    [string]$logFilePath    = "$rootFolder\Logs\"+$(get-date -format 'yyyy.MM.dd')+"_enrol.log",

    # The correct Daily Organiser file should have the PDOF9 file extension (P=Published).
    [string]$inputFilePath  = "<FOLDER_PATH>\$(Get-Date -Format 'yyMMdd')_Daily Organiser $(Get-Date -Format 'yyyy').pdof9"
...
BEGIN {
    ...
    WriteLog("$(Get-Date -format 'HH.mm.ss'): Starting daily reliefs enrollment upload process.")
...
'@
    ...
    $import_response = Invoke-RestMethod `
        -Uri "https://woodcroft.instructure.com/api/v1/accounts/1/sis_imports.json?import_type=instructure_csv" `
        -Headers $headers `
        -Method POST `
    ...
    do {
        $status_url = "https://woodcroft.instructure.com/api/v1/accounts/1/sis_imports/$upload_id"
        $status_result = Invoke-RestMethod `
            -Headers $headers `
            -Method GET `
            -Uri $status_url 
    ...
    $errors_result = Invoke-RestMethod `
        -Headers $headers `
        -Method GET `
        -Uri "https://woodcroft.instructure.com/api/v1/accounts/1/sis_imports/$upload_id/errors" `
        -FollowRelLink
...
END {
    ...
    WriteLog("`n`n$(Get-Date -format 'hh.mm.ss'): Finished.")
    WriteLog("============================================================================================================")
    WriteLog("============================================================================================================")
